...
| **P3 — Course II / v1.0** (in progress; wave 1 landed 2026-08-01) | Done: capstone simulator + scenario eval harness; stacks v0 (reference), **v1 (PF lidar localization, 6–11 cm RMSE)**, and **v2 (online mapping, no grid access)** all passing the rubric 20/20; six debugging field notes in the capstone README; `robotics_ai.planning` + `robotics_ai.mapping`; lessons 4.1, 5.1; repo on GitHub (private) with cloud CI green. Remaining (agreed order): **finish P2 tail first**, then lessons 4.2–4.4 + 5.2–5.5, then capstone v3 (dynamic obstacles, SLAM, ROS 2 packaging — WSL2/devcontainer decision), demo video, public flip + launch | End-to-end nav stack scored by `python -m eval`; 6-credit claim defensible |
| **P4 — Course III** | Modules 7 → 10 in order (perception first — leverages PointPillars background); Capstones 2–4; Isaac Lab track | 9-credit program complete |
